										...
										if (-1 === D || "string" == typeof D) {
											...
											for (var V = 0; V < R.length; V++) {
												(A = parseInt(R[V])),
													(m = []),
													(i =
														("string" == typeof D && v(h, A, T)) || C.slice());
												var G = M.matches[A];
											...
											r = s(M.matches[D] || t.matches[D], [D].concat(o), c);
										if (r) return !0;
									} else if (
										r.isQuantifier &&
...
document.addEventListener("DOMContentLoaded", () => {
	...
	if (closeFormBtns.length) {
		closeFormBtns.forEach((btn) => {
			btn.addEventListener("click", closeFormFn, false);
		});
	}
});
...
document.addEventListener("DOMContentLoaded", () => {
	...
	const dateInputs = form.querySelectorAll(".date");
	const gosNumInput = form.querySelector(".gosNum");
	const passportSeriesInput = form.querySelector(".passportSeries");
	const passportNumInput = form.querySelector(".passportNum");
	...
	Inputmask({
		regex: "[А-Я] [0-9]{3} [А-Я]{2} [0-9]{2,3}",
		placeholder: "A 000 АА 00",
	}).mask(gosNumInput);

	Inputmask({
		regex: "[0-9]{4}",
		placeholder: "0000",
	}).mask(passportSeriesInput);

	Inputmask({
		regex: "[0-9]{6}",
		placeholder: "000000",
	}).mask(passportNumInput);

	form.addEventListener("input", (e) => {
		const key = e.target.name;
		const value = e.target.value;
		formData[key] = value;
		const forLSFormData = JSON.stringify(formData);
		LS.setItem("formData", forLSFormData);
	});

	if (LS.getItem("formData")) {
		const formDataSavedValues = JSON.parse(LS.getItem("formData"));

		for (let key in formDataSavedValues) {
			form.elements[key].value = formDataSavedValues[key];
		}
	}
